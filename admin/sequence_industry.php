                ...
                <div class="col-xl-12 col-xxl-12">
                    ...
                    <div class="row">
                        ...
                        <div class="col-4 text-center">
                            <ul id="sortable-list">
                                <?php
                                $fetch_cat = $db->runQuery("SELECT * FROM `industry_type` ORDER BY sequence");
                                $no_fetch_cat = $db->numRows("SELECT * FROM `industry_type` ORDER BY sequence");
                                for ($i = 0; $i < $no_fetch_cat; $i++){
                                    ?>
                                    <li data-id="<?php echo $fetch_cat[$i]['industry_type_id'];?>"><?php echo $fetch_cat[$i]['sequence'];?> - <?php echo $fetch_cat[$i]['industry_name'];?></li>
                                    <?php
                                }
                                ?>
                            </ul>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        async function saveOrderToBackend(newOrder) {
            try {
                const response = await fetch("save_industry_order.php", {
                    method: "POST",
                    headers: {
